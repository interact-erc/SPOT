        ...
        nodes.append(
            {
                ...
                "grounded_entity": node.grounded_entity,
    ...
    for ittr, graph_orig in tqdm(enumerate(graphs), total=len(graphs)):
        ...
        for _ in range(args.grounding_per_pattern):
            ...
            while not is_grounded and external_retries < args.max_pattern_retries:
                external_retries += 1
                try:
                    grounded_graph, is_grounded = grounder.ground_graph(graph)
                except:
                    is_grounded = False

            if is_grounded:
                try:
                    sparql_writer = SPARQLQueryWriter(grounded_graph)
                    sparql_query = sparql_writer.write_query()
                    sparql_writer.set_nl_provider(sparql_provider)
                    ...
                    anon_sparql_query = sparql_writer.write_query()
                    sparql_writer.set_nl_provider(sparql_provider)
                    anon_sparql_proto_nl = sparql_writer.write_proto_nl()

                    if args.diverse_sampling:
                        grounder.update_sampled_class_rel(grounded_graph)

                    sample = {
                        "anon_proto_nl": anon_sparql_proto_nl,
                        "anon_query": anon_sparql_query,
